...
public class StoryCreator {
	...
	public static StoryData getStoryDataFromCursor(Cursor cursor) {
		...
		long rowID       = cursor.getLong(cursor.getColumnIndex(MoocSchema.Story.Cols.ID));
		long loginId     = cursor.getLong(cursor.getColumnIndex(MoocSchema.Story.Cols.LOGIN_ID));
		long storyId     = cursor.getLong(cursor.getColumnIndex(MoocSchema.Story.Cols.STORY_ID));
		String title     = cursor.getString(cursor.getColumnIndex(MoocSchema.Story.Cols.TITLE));
		String body      = cursor.getString(cursor.getColumnIndex(MoocSchema.Story.Cols.BODY));
		String audioLink = cursor.getString(cursor.getColumnIndex(MoocSchema.Story.Cols.AUDIO_LINK));
		String videoLink = cursor.getString(cursor.getColumnIndex(MoocSchema.Story.Cols.VIDEO_LINK));
		String imageName = cursor.getString(cursor.getColumnIndex(MoocSchema.Story.Cols.IMAGE_NAME));
		String imageMetaData = cursor.getString(cursor.getColumnIndex(MoocSchema.Story.Cols.IMAGE_LINK));
		String tags      = cursor.getString(cursor.getColumnIndex(MoocSchema.Story.Cols.TAGS));
		long creationTime = cursor.getLong(cursor.getColumnIndex(MoocSchema.Story.Cols.CREATION_TIME));
		long storyTime   = cursor.getLong(cursor.getColumnIndex(MoocSchema.Story.Cols.STORY_TIME));
		double latitude  = cursor.getDouble(cursor.getColumnIndex(MoocSchema.Story.Cols.LATITUDE));
		double longitude = cursor.getDouble(cursor.getColumnIndex(MoocSchema.Story.Cols.LONGITUDE));

		// construct the returned object
		StoryData rValue = new StoryData(rowID, loginId, storyId, title, body, audioLink, videoLink, imageName, imageMetaData, tags,
